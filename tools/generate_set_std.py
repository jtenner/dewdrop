...
SOURCE = ROOT / "std/set.dew"
TARGET = ROOT / "src/standard_sources/standard_set_sources.mbt"
...
def rendered_source() -> str:
    ...
    return (
        "///|\n"
        "pub fn standard_set_source() -> Bytes {\n"
        f"  {literal}\n"
        "}\n"
    )
...
def main() -> None:
    ...
    if args.check:
        if not TARGET.exists() or TARGET.read_text(encoding="utf-8") != rendered:
            raise SystemExit(
                "src/standard_sources/standard_set_sources.mbt is stale; "
                "run tools/generate_set_std.py"
            )
