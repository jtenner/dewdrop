#!/usr/bin/env python3
"""Embed std/map.dew as the compiler-owned bootstrap source."""

import argparse
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
SOURCE = ROOT / "std/map.dew"
TARGET = ROOT / "src/standard_sources/standard_map_sources.mbt"
...
def rendered_source() -> str:
    ...
    return (
        "///|\n"
        "pub fn standard_map_source() -> Bytes {\n"
        f"  {literal}\n"
        "}\n"
    )
...
def main() -> None:
    ...
    if args.check:
        if not TARGET.exists() or TARGET.read_text(encoding="utf-8") != rendered:
            raise SystemExit(
                "src/standard_sources/standard_map_sources.mbt is stale; "
                "run tools/generate_map_std.py"
            )
