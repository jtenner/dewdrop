#!/usr/bin/env python3
"""Benchmark private scalar unit enums against retained subtype-family enums."""
...
TMP = ROOT / ".tmp" / "scalar-unit-enum-benchmark"


def source(cases: int, public: bool) -> str:
    visibility = "pub " if public else ""
    calls = " +\n    ".join(f"classify(value > {index})" for index in range(cases))
...
def main() -> None:
    ...
    expected = min(41, args.cases) + 2 * max(args.cases - 41, 0)
    ...
    print(json.dumps({
        ...
        "scalar_struct_new": wat_count(scalar, "struct.new"),
        "retained_struct_new": wat_count(retained, "struct.new"),
        "scalar_struct_get": wat_count(scalar, "struct.get"),
        "retained_struct_get": wat_count(retained, "struct.get"),
        "scalar_ref_cast": wat_count(scalar, "ref.cast"),
        "retained_ref_cast": wat_count(retained, "ref.cast"),
