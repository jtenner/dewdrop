...
grep -q '^standard interface cache: hit$' \
  .tmp/dew-external-package-cache-hit.txt
test "$(find .tmp/dew-external-package-cache/interfaces -name 'v10-*.dwi' | wc -l)" -eq 1
...
grep -q '^standard interface cache: miss$' \
  .tmp/dew-external-package-cache-changed.txt
test "$(find .tmp/dew-external-package-cache/interfaces -name 'v10-*.dwi' | wc -l)" -eq 2
tools/dew build \
  --manifest tests/abi-consumers/imported-package/dew.json \
...
node tools/dew-wasm-consumer.mjs \
  ...
  run i32 \
  fixture.application \
  6132bf2ed8a4dd875d52321e5ad4e8a7cfa8c1c3e7f13f1f74615aa2bb1d8706 \
