...
grep -q '^standard interface cache: hit$' \
  .tmp/dew-external-package-cache-hit.txt
test "$(find .tmp/dew-external-package-cache/interfaces -name 'v9-*.dwi' | wc -l)" -eq 1
...
grep -q '^standard interface cache: miss$' \
  .tmp/dew-external-package-cache-changed.txt
test "$(find .tmp/dew-external-package-cache/interfaces -name 'v9-*.dwi' | wc -l)" -eq 2
tools/dew build \
  --manifest tests/abi-consumers/imported-package/dew.json \
...
node tools/dew-wasm-consumer.mjs \
  ...
  run i32 \
  fixture.application \
  294ac885b58d8b9a7b4dea0e606bb493404a6dce7861fa6360d5eafa4fd1dab4 \
