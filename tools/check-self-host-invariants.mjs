...
const probes = [
  ...
  {
    name: "SPC-301 unresolved call result presence requires its type",
    expected: [301, 7, 5100n, (5100n << 32n) | 3n, (5100n << 32n) | 1n, 6, 0n, 2n, 0n],
  },
  ...[
  ...
  ...[
    ...
    ["ARN-101 specialized type substitution checks product child IDs", 101, 2n, 2147483647n, 1n],
  ].map(([name, code, expected, actual, detail]) => ({
    name,
  ...
  ...[
    ...
    ["SPC-301 nested resolved callback results cannot hide errors", 301, 0n, 2n, 4294967296n],
    ["ARN-108 nested resolved callback results cannot contain cycles", 108, 0n, 1n, 4294967296n],
    ["SPC-301 nested inferred callback results cannot hide errors", 301, 0n, 2n, 4294967296n],
    ["ARN-108 nested inferred callback results cannot contain cycles", 108, 0n, 1n, 4294967296n],
    ["ARN-101 nested callback type spans remain checked", 101, 0n, 0n, 4294967295n],
    ["SOL-207 nested callback resolved links cannot overflow encoding", 207, 2147483646n, 4294967295n, 4294967296n],
    ["SPC-301 nested callback projections cannot remain unresolved", 301, 0n, 3n, 4294967296n],
    ["SPC-301 nested callback variables must be zonked", 301, 0n, 2n, 4294967296n],
  ].map(([name, code, expected, actual, detail]) => ({
    name,
