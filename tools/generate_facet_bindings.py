...
NAMESPACES = [
    ("core", "std/facet.dew", "dew.std.facet"),
    ...
    ("fs_preopen", "std/facet/fs/preopen.dew", "dew.std.facet.fs.preopen"),
    ("fs_descriptor", "std/facet/fs/descriptor.dew", "dew.std.facet.fs.descriptor"),
    ("fs_read", "std/facet/fs/read.dew", "dew.std.facet.fs.read"),
    ("fs_write", "std/facet/fs/write.dew", "dew.std.facet.fs.write"),
    ("fs_position", "std/facet/fs/position.dew", "dew.std.facet.fs.position"),
    ("fs_sync", "std/facet/fs/sync.dew", "dew.std.facet.fs.sync"),
    ("fs_path", "std/facet/fs/path.dew", "dew.std.facet.fs.path"),
    ("fs_directory", "std/facet/fs/directory.dew", "dew.std.facet.fs.directory"),
    ("fs_link", "std/facet/fs/link.dew", "dew.std.facet.fs.link"),
    ("net_socket", "std/facet/net/socket.dew", "dew.std.facet.net.socket"),
    ("net_datagram", "std/facet/net/datagram.dew", "dew.std.facet.net.datagram"),
    ("net_dns", "std/facet/net/dns.dew", "dew.std.facet.net.dns"),
...
def namespace_for(name: str) -> str:
    if name in {"abi_version", "handle_close", "proc_exit", "proc_yield"}:
        return "core"
    if name.startswith(("stdio_", "args_", "env_")):
        return "process"
    if name.startswith("clock_") or name.startswith("sleep_"):
        return "clock"
    if name.startswith("random_"):
        return "random"
    if name.startswith("fs_preopen_"):
        return "fs_preopen"
    if name in {"fd_rights", "fd_get_flags", "fd_set_flags", "fd_stat"}:
        return "fs_descriptor"
    if name.startswith(("fd_read", "fd_pread")):
        return "fs_read"
    if name.startswith(("fd_write", "fd_pwrite")):
        return "fs_write"
    if name in {"fd_seek", "fd_tell", "fd_set_size"}:
        return "fs_position"
    if name in {"fd_sync", "fd_datasync"}:
        return "fs_sync"
    if name.startswith(("path_open_", "path_stat_", "path_create_dir_", "path_remove_", "path_rename_")):
        return "fs_path"
    if name.startswith("dir_iter_"):
        return "fs_directory"
    if name.startswith(("path_link_", "path_symlink_", "path_readlink_")):
        return "fs_link"
    if name in {
        "socket_open",
        "socket_bind",
        ...
        return "net_socket"
    if name.startswith(("socket_recvfrom_", "socket_sendto_")):
        return "net_datagram"
...
def render_root(items: list[Import]) -> str:
    lines = [
        "// Generated by tools/generate_facet_bindings.py. Do not edit.",
        "",
    ...
    lines.append("")
    lines.extend(render_module_body("core", items, include_header=False))
    return "\n".join(lines).rstrip() + "\n"
...
def render_module_body(namespace: str, imports: list[Import], include_header: bool = True) -> list[str]:
    ...
    host_name = "Facet" + "".join(part.title() for part in namespace.split("_")) + "Host"
    lines.append(f'foreign impl "facet" as {host_name} {{')
    helpers: set[str] = set()
    for item in items:
        names = parameter_names(item)
        if len(names) != len(item.params):
            raise ValueError(f"parameter-name count mismatch for {item.name}")
        params = ", ".join(f"{name}: {dew_scalar_type(item, index, token)}" for index, (name, token) in enumerate(zip(names, item.params)))
        lines.append(f'  fn raw_{item.name}({params}) -> {host_return_type(item)} = "{item.name}"')
        kind = raw_result_kind(item)
        if kind in RESULT_FIELDS:
            ...
            suffix = kind.rsplit("_", 1)[1]
            helpers.update({f"array_{suffix}_result_value", f"array_{suffix}_result_error"})
        elif kind.startswith("dir_array_"):
            suffix = kind.rsplit("_", 1)[1]
            helpers.update(f"dir_array_{suffix}_result_{field}" for field in ("value", "file_type", "inode", "done", "error"))
    for helper in sorted(helpers):
        ...
        if scalar_match is not None:
            ...
            lines.append(
                f'  fn raw_{helper}(value: {RAW_RESULT_TYPES[kind]}) -> {output} = "{helper}"'
            )
            continue
        ...
        if match:
            ...
            lines.append(f'  fn raw_{helper}(value: {input_type}) -> {output} = "{helper}"')
            continue
        match = re.fullmatch(r"dir_array_(i8|i16|i32)_result_(value|file_type|inode|done|error)", helper)
        assert match is not None
        suffix, field = match.groups()
        input_type = "FacetDirectoryArray" + suffix.upper() + "Result"
        output = ARRAY_DEW_TYPES[suffix] if field == "value" else ("U64" if field == "inode" else "U32")
        lines.append(f'  fn raw_{helper}(value: {input_type}) -> {output} = "{helper}"')
    lines.extend(["}", ""])
    ...
    for item in items:
        ...
        args = ", ".join(names)
        call = f"{host_name}.raw_{item.name}({args})"
        kind = raw_result_kind(item)
        if kind == "direct":
            lines.append(f"  {call}")
            ...
            for field, _dew_type in RESULT_FIELDS[kind]:
                lines.append(f"    {field}: {host_name}.raw_{prefix}_{field}(result)")
            lines.append("  }")
        elif kind.startswith("array_"):
            suffix = kind.rsplit("_", 1)[1]
            lines.append(f"  let result = {call}")
            lines.append(f"  let error = {host_name}.raw_array_{suffix}_result_error(result)")
            lines.append("  if error == 0u32 {")
            lines.append(f"    Result::Ok({host_name}.raw_array_{suffix}_result_value(result))")
            lines.append("  } else {")
            ...
            lines.append(f"  let result = {call}")
            lines.append(f"  let error = {host_name}.raw_dir_array_{suffix}_result_error(result)")
            lines.append("  if error != 0u32 {")
            lines.append("    Result::Err(error)")
            lines.append(f"  }} else if {host_name}.raw_dir_array_{suffix}_result_done(result) != 0u32 {{")
            lines.append("    Result::Ok(Option::None)")
            lines.append("  } else {")
            lines.append(f"    Result::Ok(Option::Some({entry_type}::{{")
            lines.append(f"      name: {host_name}.raw_dir_array_{suffix}_result_value(result)")
            lines.append(f"      file_type: {host_name}.raw_dir_array_{suffix}_result_file_type(result)")
            lines.append(f"      inode: {host_name}.raw_dir_array_{suffix}_result_inode(result)")
            lines.append("    }))")
            lines.append("  }")
        lines.extend(["}", ""])
    return lines
...
def render_dew_modules(imports: list[Import]) -> dict[Path, str]:
    output: dict[Path, str] = {ROOT / "std/facet.dew": render_root(imports)}
    for namespace, relative, _module in NAMESPACES[1:]:
...
def render_smoke(imports: list[Import]) -> str:
    aliases = {namespace: "@" + namespace for namespace, _, _ in NAMESPACES}
    lines = ["// Generated by tools/generate_facet_bindings.py. Do not edit."]
    for namespace, _relative, module in NAMESPACES:
