#!/usr/bin/env python3
"""Benchmark pure-gap sole-use scalar replacement against an escaping helper."""
...
def main() -> None:
    ...
    optimized = build("pure-gap", source(args.iterations, True))
    baseline = build("escaping-helper", source(args.iterations, False))
    raw = measure([optimized, baseline], args.iterations, args.samples, args.batch)
    ...
    print(json.dumps({
        ...
        "batch": args.batch,
        "pure_gap_median_us": round(optimized_us, 3),
        "escaping_helper_median_us": round(baseline_us, 3),
        "pure_gap_to_escaping_ratio": round(optimized_us / baseline_us, 4),
        "pure_gap_wasm_bytes": optimized.stat().st_size,
        "escaping_helper_wasm_bytes": baseline.stat().st_size,
        "pure_gap_struct_news": instruction_count(optimized, "struct.new"),
