...
The ambient `Debug` trait and `debug(value)` facade stream deterministic output through bounded partial-write-safe WASI stdout. Integer decimal formatting is ordinary Dew code with fixed scratch storage, exact narrow-width normalization, and signed-minimum handling; it performs no GC allocation. Float formatting also runs in Dew and emits exact lowercase IEEE bits, including signed zero and NaN payloads, using only one-to-one reinterpret instructions to read those bits. V128 formatting uses the same Dew digit writer with two one-to-one lane reads, high lane first. Primitive implementations cover Unit, Bool, all fixed-width numeric types, String, Bytes, SWAR carriers, and V128. Structs and enums may use postfix `derive(Debug)`; `derive(Eq)`, `derive(Hash)`, and `derive(Show)` are also implemented with conditional generic prerequisites and ordinary coherence/import rules. `Show` remains non-ambient and side-effect free: `show(value) -> String` and builder-backed composition use explicit evidence with deterministic depth and byte limits. Array, FixedArray, Option, and Result have structural deterministic Show forms. `render_compiler_diagnostic` produces bounded `path:line:column: level[code]: message (byte offset)` output without ambient Debug.
...
`Iter<t>` is declared with `Option<t>` and exposes only `next(self) -> Option<t>`. Exhaustion returns `None` repeatedly. `for pattern in iterable { ... }` calls `iterable.iter()` once and consumes `next()` until `None`; the compiler lowers it to the existing functional loop. The native backend scalarizes immediate Option construction and matching to locals, while escaping Option values retain the normal WasmGC enum representation.

The self-host compiler uses the declared enum fragments for Option and Result, with the same storage rules as user enums. Generic scalar payloads have explicit boxes; Unit uses a storage marker. It has no private Option/Result runtime types, hidden Result error-identity field, or Some/None/Ok/Err physical name dispatch. Native scalarization and allocation parity are separate work.
...
The module is generated from the compiler's backend inline-builtin surface. It exposes every currently source-addressable scalar, conversion, memory, packed-lane, and SIMD operation with a `wasm_`-prefixed public name, while retaining `i64_trunc_i32` as the first-published compatibility spelling. `wasm_bytes_load_u8x16(Bytes, U32)` is an ordinary Dew function: it checks the logical range, reads sixteen bytes, and assembles the vector with one-to-one lane instructions. Unlike `wasm_v128_load(U32)`, it does not address linear memory. The shared byte-storage accessors still need their separate migration. The generator fails when a backend inline builtin lacks a declaration, and the bootstrap source is generated from the same output.
...
`dew.std.wasm.wasi` exposes all 46 raw `wasi_snapshot_preview1` functions with exact U32/U64/I64 ABI carriers. These are public [foreign host imports](../docs/foreign-imports.md), not builtins. It requires an explicit import and is not selected by `open dew.std.*`, so the broad convenience wildcard does not retain low-level host signatures. Its U32 pointers address exported linear memory, and callers own Preview 1 record and buffer layouts. Import it with `import dew.std.wasm.wasi as @wasi`. Reachable raw calls retain their exact WASI field names; unreachable calls are removed.
...
The read/write loops are ordinary Dew functions that call the raw foreign imports. WASI marshalling uses the 65,520 data bytes remaining in one reusable 64 KiB linear-memory page. Writes continue through valid partial host writes. Reads continue after full windows and stop at the requested limit, EOF, or a short read. Nonzero errno and invalid host progress trap; linear memory never owns the resulting Bytes. The shared scratch page is reserved for each synchronous operation; host callbacks must not re-enter this adapter. `Bytes.find` and `contains` specialize empty and one-byte needles, use SIMD candidate filtering for short needles, and rolling fingerprints with exact verification for long needles. `starts_with` and `ends_with` compare exact logical ranges without wrappers or copies. `StringBuilder` and `BytesBuilder` grow private GC storage geometrically, publish with `finish()`, and trap on later use. StringBuilder accepts String/StringView, checked ASCII bytes, Unicode scalar values, and strictly validated Bytes; BytesBuilder accepts arbitrary Bytes and individual U8 values.
