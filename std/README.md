...
The self-host compiler uses Option's declared enum fragments, with the same storage rules as user enums. Generic scalar payloads have explicit boxes; Unit uses a storage marker. It has no private Option runtime types or Some/None physical name dispatch. Native scalarization and allocation parity are separate work.
