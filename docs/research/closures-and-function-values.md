...
Parser types preserve ordered parameter types, result type, and source offset. Collection lowers them iteratively into `FunctionTypeSyntax`, reusing the module type-argument arena for the parameter span. Resolution interns `FunctionType` nodes structurally and traverses them through generic alias substitution, alias normalization, imported-interface translation, visibility checks, coherence signature comparison, nominal reachability, and frozen-interface cache version 1. Every callable also receives a canonical structural function-type ID after signature normalization and a version 1 SHA-256 ABI fingerprint using stable dependency declaration identities.
...
Lambda parameter and result syntax resolves in the enclosing root declaration's generic/type scope. Each lambda receives one canonical structural `FunctionType`, with alias-normalized parameter and result types, and the frozen-interface cache is version 1; it retains resolved signature tables and version 1 content-sensitive callable, nominal, and module-interface fingerprints with dependency content records while rejecting orphan implementation evidence.
...
Escaping bounded generic named references also reuse the common closure base. Semantic link planning preserves the reference's frozen obligation evidence, resolves an exact target specialization for each caller specialization, and records any caller-local runtime dictionary parameter indices. The backend emits one final subtype per evidence-aware reference identity. Its inherited entry field contains a private environment-first trampoline; subsequent immutable `eqref` fields contain dictionaries in source-bound order. Construction emits the trampoline `ref.func`, then each captured dictionary local, then one `struct.new`. Invocation follows the ordinary closure ABI. Inside the trampoline, visible arguments are loaded before evidence fields, the environment is cast to the exact subtype, and the exact local or imported target specialization is called directly. A closed generic caller still receives a zero-capture subtype and trampoline so references inside generic bodies cannot accidentally select a carrier-equivalent specialization with different evidence. Public all-reference callback factories therefore preserve consumer-supplied dictionaries after return without changing their structural result ABI.
...
A deterministic post-lowering use pass now directizes immediate lambdas and immutable local function values whose every use is a call and which are not captured. Named references become ordinary direct calls. Non-escaping lambdas receive lifted signatures containing their captures before source parameters, and call sites pass capture sources directly. Their closure construction, local initialization, entry load, function-type test, casts, and `call_ref` are omitted. Unused local lambda and named-reference initializers are removed entirely; unused lambda entry functions and physical closure subtypes are not emitted. Program-wide callback summaries additionally follow immutable callback aliases and transitive forwarding wrappers across local/imported public APIs. Ignored wrapper preludes may be removed only when their complete call tree is summarized as total and discardable. Public wrapper bodies remain available through the ordinary closure ABI, while returned, captured, multiply invoked, trapping, recursive, mutating, and unknown callback uses conservatively retain dynamic dispatch.
...
1. Extend alternate-ABI specialization only if measured workloads justify directizing conditionally or multiply invoked callbacks; those uses currently remain dynamic and semantically exact.
2. Define recursive generic aggregate layout sharing while preserving exact callback signatures.
3. Remove unused signature, evidence-trampoline, and closure-subtype plans after directization and reachability pruning.
4. Continue package artifact recovery and compatibility hardening without changing the V1 runtime-evidence ABI.
5. Measure closure allocation, cell, call, cast, and capture-load costs in Node and Wago with focused runtime loops.
...
- Expected-type generic function references retain inferred type arguments and frozen obligation evidence. Program linking transitively closes nested demands and materializes one deterministic function body per canonical physical-carrier-and-evidence identity, including imported definitions, multiple type parameters, and escaping references. Ordinary closed references may use specialized singletons; references inside generic specializations use exact evidence trampolines, with immutable dictionary fields only when caller-local runtime evidence must escape. Unspecialized recipes are not executable or exported; root erased fallbacks, evidence captures, and structural callback wrappers use the same flattened closure ABI.
