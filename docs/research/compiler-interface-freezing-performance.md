...
Interface freezing scans declarations first and runs `infer_frozen_module_values` only when the module actually publishes a top-level `let`. The classifier uses the same visibility rules as interface construction. The September 8 correctness update removed the old standard-module publication exception: exported library declarations now have explicit `pub` markers. Private and test-only lets do not force interface-time body inference in any module. Public lets continue to receive their inferred semantic type, and ordinary later body analysis remains unchanged. See the [visibility update](module-source-visibility-2026-09-08.md).
...
- version 1 callable, nominal, module-content, and graph fingerprint domains;
