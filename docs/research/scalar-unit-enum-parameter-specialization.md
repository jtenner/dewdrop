...
A bounded packed ABI for private enums with unit or one `I32` tuple payload is implemented in `scalar-payload-enum-specialization.md`. Public/generic signatures, function values, methods, imported calls, multi-field or non-`I32` payloads, and adapter/versioning rules remain separate work.
