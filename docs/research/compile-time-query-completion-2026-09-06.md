...
that selection until an exact instance exists. It is not an error, a reference
carrier, or a guessed target. The inferred-body codec preserves the selection;
the emitter rejects it if it ever reaches emission.

An affected instance rechecks source HIR after branch selection. It keeps source
IDs and a separate reachability mask for expressions, blocks, patterns, and
locals. It does not replace dead expressions with Unit. Private source-type
substitutions preserve canonical primitive/nominal identities and leave callable
signatures generic, including recursive call signatures. This was necessary:
appending a duplicate I64 type caused an operator lookup failure because the
solver relies on interned leaf identities.

The new pass rebuilds inference and lowering in private arenas before the
physical plan is frozen. It runs only for a body with deferred checks. The first
implementation rechecks its module, not only its body family; reducing that cost
is follow-up performance work, not a reason to weaken checks.

Native execution passes 79 checks (9.957 s generation, 0.030 s execution).
Four focused tests pass: selected field access, selected integer operators,
a source-located CT-041 error for a missing selected field, and an independent
error in an unused generic branch. Deferred selections also round-trip through
