...
A public root generic whose symbolic bounds are used only to construct erased trait values receives an evidence-carrying all-reference fallback. Ordinary physical parameters remain first and one `eqref` vtable parameter per source-ordered bound is appended afterward. The fallback casts each hidden parameter to the exact typed vtable before constructing the trait object. A core-Wasm consumer can therefore define structurally equivalent trait/vtable/object types and supply its own receiver and `ref.func` implementation without exposing GC references to JavaScript. Bodies with symbolic static bound calls or nested generic obligations are rejected from this fallback tranche rather than receiving an invalid evidence-free export.
