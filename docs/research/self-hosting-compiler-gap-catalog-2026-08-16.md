...
The linked Starshine ABI, product values, allocation-free `for`, tokenizer, full parser, packed semantic IDs, declaration collection, flat HIR, resolved type interning, local and imported signature resolution, alias SCC analysis, generic alias substitution, transparent normalization, signature-level module interfaces/imports, body name resolution, deterministic derive expansion, imported callable, aggregate, module-value, and implementation recipes, rollback-capable local unification, and the basic per-body inference core are complete.

1. **Port semantic phases in dependency order.** Complete callable/member/operator selection, generic instantiation, trait evidence, lambda-family and module-value inference, result compaction, and body flow; then continue with layouts, specialization, optimization, and linking.
