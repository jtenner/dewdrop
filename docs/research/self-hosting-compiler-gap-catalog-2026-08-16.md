...
The linked Starshine ABI, product values, allocation-free `for`, tokenizer, full parser, packed semantic IDs, declaration collection, flat HIR, resolved type interning, local and imported signature resolution, alias SCC analysis, generic alias substitution, transparent normalization, signature-level module interfaces/imports, and body name resolution are complete.

1. **Port semantic phases in dependency order.** Continue with derive expansion and imported semantic recipes; then inference, flow, layouts, specialization, optimization, and linking.
