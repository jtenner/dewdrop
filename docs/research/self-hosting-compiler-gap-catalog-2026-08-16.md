...
The linked Starshine ABI, product values, allocation-free `for`, tokenizer, full parser, packed semantic IDs, declaration collection, flat HIR, resolved type interning, local and imported signature resolution, alias SCC analysis, generic alias substitution, transparent normalization, signature-level module interfaces/imports, body name resolution, deterministic derive expansion, imported callable, aggregate, module-value, and implementation recipes, rollback-capable local unification, the basic per-body inference core, iterative generic signature instantiation, local direct callable/overload selection, local fields, struct construction, inherent methods, enum values, trait-backed operators, and index selection are complete.

1. **Port semantic phases in dependency order.** Complete imported selections, struct enum variants and pattern selections, implementation coherence, trait evidence, lambda-family and module-value inference, result compaction, and body flow; then continue with layouts, specialization, optimization, and linking.
