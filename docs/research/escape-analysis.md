...
| preserved nonconstant guard plan | 0.0155 µs | 0 | 0 | 467 |
| exact tuple payload scalar transform | 0.0165 µs | 0 | 0 | 455 |
| exact struct payload scalar transform | 0.0164 µs | 0 | 0 | 455 |
| let-bound tuple scalar transform | 0.0154 µs | 0 | 0 | 455 |
| immutable-local block scalar transform | 0.0192 µs | 0 | 0 | 463 |
...
Direct, reordered-arm, alternative-arm, constant-guard, stable-guard, composite-guard, payload-guard, preserved-guard plan, scalar-transform, let-bound transform, block-transform, false-catchall, selected-catchall, binding-forward, nested binding-projection, and let-bound direct tuple paths measured 0.9478x, 1.2095x, 0.9071x, 0.9884x, 0.9768x, 0.9768x, 0.9942x, 0.9066x, 0.9594x, 0.8955x, 0.9747x, 0.9884x, 0.9826x, 1.0000x, 0.9710x, and 0.9942x the retained runtime on their documented runs. Binding forwarding is tied but removes six Wasm bytes and the first match's dispatch while intentionally retaining the one live variant; nested projection removes every variant operation. Direct struct, stable-guard struct, struct-transform, and nested elimination measured 0.9654x, 0.9596x, 0.9711x, and 0.9173x. Nested elimination removed 134 Wasm bytes, two allocations, and six aggregate reads.
...
Transforms with multiple payload bindings, mutable/captured or ambiguous local chains, effectful block items, trapping/unsupported operations, effectful or user-defined operator guards, nested/non-direct payload patterns, unknown or catchall constructor relations, missing terminal arms, and reference/generic result carriers remain conservative. Alternative arms require exactly one constructor-compatible branch, and nested paths are limited to fresh struct and tuple-struct constructors with one unambiguous scalar binding.
