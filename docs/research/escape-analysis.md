...
Direct, reordered-arm, alternative-arm, constant-guard, false-catchall, selected-catchall, binding-forward, and let-bound tuple paths measured 0.9641x, 0.9283x, 0.9103x, 0.9522x, 0.9701x, 0.9641x, 1.0239x, and 0.9641x the retained runtime. Binding forwarding is effectively tied but removes six Wasm bytes and the first match's dispatch while intentionally retaining the one live variant. Direct struct and nested elimination measured 0.9885x and 0.9880x. Nested elimination removed 134 Wasm bytes, two allocations, and six aggregate reads.
...
Binding-derived scalar selection and nonconstant matching-constructor guard planning remain conservative. Alternative arms require exactly one constructor-compatible branch, and nested paths are limited to fresh struct and tuple-struct constructors with one unambiguous scalar binding.
