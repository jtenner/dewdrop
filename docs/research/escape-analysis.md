...
| immutable tuple local | 0.0162 µs | 0 | 0 | 440 |
...
Direct, reordered-arm, alternative-arm, constant-guard, false-catchall, selected-catchall, and let-bound tuple elimination measured 0.9701x, 0.9103x, 0.9097x, 0.9880x, 0.9821x, 0.9641x, and 0.9701x the retained runtime. Direct struct and nested elimination measured 0.9885x and 0.9880x. Nested elimination removed 134 Wasm bytes, two allocations, and six aggregate reads.
...
Binding-dependent wildcard/binding selection and nonconstant matching-constructor guard planning remain conservative. Alternative arms require exactly one constructor-compatible branch, and nested paths are limited to fresh struct and tuple-struct constructors with one unambiguous scalar binding.
