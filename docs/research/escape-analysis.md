...
Direct, reordered-arm, alternative-arm, constant-guard, false-catchall, selected-catchall, binding-forward, nested binding-projection, and let-bound tuple paths measured 0.9582x, 0.9044x, 0.9044x, 0.9767x, 0.9761x, 0.9641x, 1.0000x, 0.9701x, and 0.9641x the retained runtime. Binding forwarding is tied but removes six Wasm bytes and the first match's dispatch while intentionally retaining the one live variant; nested projection removes every variant operation. Direct struct and nested elimination measured 0.9885x and 0.9880x. Nested elimination removed 134 Wasm bytes, two allocations, and six aggregate reads.
...
Arbitrary binding-derived scalar transforms and nonconstant matching-constructor guard planning remain conservative. Alternative arms require exactly one constructor-compatible branch, and nested paths are limited to fresh struct and tuple-struct constructors with one unambiguous scalar binding.
