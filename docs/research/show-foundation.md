...
Standard evidence covers Unit, Bool, every signed and unsigned fixed-width integer, String, and StringView. Unit uses `()`; integers use base-ten ASCII with no leading zeroes or positive sign, and signed minima are handled without overflowing their signed carrier. Bool uses `true` and `false`; String and StringView append their text without quoting or escaping. Generic Unit specializations omit the physical Unit parameter while preserving subsequent parameter indices. Floating-point, Bytes, SWAR, and SIMD lane formats remain explicit follow-up work rather than receiving accidental unstable spellings.
