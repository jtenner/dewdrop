...
The collector diagnoses empty and duplicate module/file paths. Strict `dew.json` loading and ordered compiler-owned `dew.std` package roots are implemented by the bootstrap driver. Versioned external dependencies and lockfiles remain later work.
