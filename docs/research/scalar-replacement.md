...
| synthesized generic components | 0.0140 µs | 0 | 0 | 473 |
| retained generic aggregate | 0.0152 µs | 1 | 2 | 590 |
| synthesized nominal references | 0.0138 µs | 2 | 2 | 482 |
| retained outer reference aggregate | 0.0149 µs | 3 | 4 | 517 |

Scalar and mixed forms were effectively tied within timer noise. Mixed replacement removed 44 Wasm bytes, one allocation, and two field reads. Generic splitting measured 0.9212x and removed 117 bytes plus every aggregate operation. Source-local reuse removed 380–384 bytes; synthesized direct scalars removed 154 bytes, one allocation, and 16 field reads. Exact nominal-reference splitting measured 0.9263x its retained baseline and removed the outer aggregate allocation, two outer field reads, and 35 Wasm bytes while retaining both inner objects.
