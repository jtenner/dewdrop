...
| retained direct aggregate | 0.0190 µs | 1 | 16 | 783 |
| mixed alias/direct components | 0.0168 µs | 0 | 0 | 445 |
| retained mixed aggregate | 0.0167 µs | 1 | 2 | 489 |
| synthesized nominal references | 0.0159 µs | 2 | 2 | 482 |
| retained outer reference aggregate | 0.0174 µs | 3 | 4 | 517 |

Scalar forms, including mixed alias/direct replacement, were effectively tied within timer noise. Mixed replacement removed 44 Wasm bytes, one allocation, and two field reads. Source-local component reuse removed 380–384 bytes; synthesized direct scalars removed 154 bytes, one allocation, and 16 field reads. Exact nominal-reference splitting measured 0.9139x its retained baseline and removed the outer aggregate allocation, two outer field reads, and 35 Wasm bytes while retaining both inner objects.
...
The allocation-free `if` and match joins measured 0.9550x and 0.9566x their retained baselines, roughly 4.4–4.5% faster, and removed 38–40 Wasm bytes. The match fixture's remaining struct operations belong to the matched `Choice` enum rather than `Pair`. Duplicate/mutable/nonlocal aliases, fresh reference constructors, and erased-generic components still require broader identity-aware planning.
