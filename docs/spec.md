...
| D-579 | Implemented | A nonconstant guard on a known matching constructor may be eliminated when it is a discardable direct local/capture read and every possible fallthrough arm for that constructor returns the identical direct payload binding. Since either guard outcome selects the same source-ordered constructor component, guard evaluation and match dispatch are observationally redundant. Effectful guards, differing payloads, catchalls, or missing fallback remain conservative. |
