...
| D-482 | Implemented and cache-refined | Lambda signatures resolve in the enclosing generic/type scope and persist through the current V11 frozen-interface/cache format. Isolated body inference and executable closure emission are implemented by later decisions. |
...
| D-507 | Superseded by D-524, D-525, and the V11 interface/cache decisions | Exported callables now use V4 fingerprints, public nominals and module content use content-sensitive V2 fingerprints, transitive interfaces ignore assigned semantic IDs, and `dew.abi` performs compatibility negotiation. |
