...
| D-526 | Implemented provisionally | A versioned `dew.json` package declares exact package name and semantic version. Each dependency declares package name, exact semantic version, relative manifest path, canonical `sha256-<hex>` source integrity, and the expected 64-hex transitive interface fingerprint. Resolution is recursive, workspace-bounded, cycle rejecting, and conflict rejecting; dependency modules precede root modules deterministically. The compiler verifies every resolved or cache-injected dependency root interface against that expectation before lowering. The resolved package closure produces a domain-separated dependency-interface cache key. Cache V7 injects compiler-owned standard and non-root dependency interfaces, rejects corrupt or identity-mismatched artifacts, reports deterministic miss/hit behavior, and creates a distinct artifact when dependency integrity changes. |
