...
> **Status:** Static dispatch and runtime trait values for object-safe traits over nominal reference, scalar, packed, and SIMD receivers are implemented across linked modules. Concrete-to-trait boundaries retain exact coherent evidence, reconstruct local or imported requirement signatures, construct WasmGC trait objects using one shared typed dictionary per exact evidence tree, and invoke source-ordered methods with `call_ref`. Closed generic implementation dictionaries and prerequisite-aware exact-flow devirtualization execute through locals, same-evidence branches, parameter returns, and private forwarding chains. Symbolic erasure inside open generic bodies remains tracked in `docs/roadmap.md`.
