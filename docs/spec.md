...
`dew.std.queue` defines mutable FIFO `Queue<t>` with construction, exact reservation, enqueue, optional dequeue, optional non-mutating front peek, length, emptiness, clear, and explicit iterator functions. Empty dequeue and peek return `Option::None`; aliases observe mutation; iteration is stable front-to-back. The current carrier-specialized contiguous representation shifts the live prefix on dequeue. This is deterministic but O(n), and the public API is representation-independent so the circular-buffer milestone can replace shifting without source changes.
