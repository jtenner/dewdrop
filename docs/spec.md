...
| D-478 | Implemented provisionally | Declaration type positions parse `fn(T, ...) -> R`, including zero parameters and nested function results. Collection lowers these types iteratively into `FunctionTypeSyntax` with an ordered parameter span and exact result type. Resolution visits child types and emits `UnsupportedFunctionType` until the semantic function type and closure ABI are implemented. |
