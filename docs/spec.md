...
A call whose target is a module value name reads the frozen source-ordered value binding. One callable declaration selects directly. Function types are structurally resolved. Unambiguous non-generic local/imported functions may be selected as values, and calls through function-typed locals/module values are checked against their structural parameter and result types. Reachable concrete signatures lower to canonical typed Wasm function references; named values emit `ref.func`, and value calls emit `call_ref`. Program links structurally coalesce equivalent signatures across modules so imported definitions and consumer-side function types share one final Wasm type identity. Field and qualified call targets defer to the later implementation/member phase described in Section 3.14.
...
WasmGC planning emits one closure struct for modules containing lambdas: immutable nullable abstract-function and `eqref` environment fields. Every lambda receives a deterministic environment struct and capture-to-physical-field relocation. Scalar/SIMD captures use compact storage, references use GC-safe `eqref`, and Unit/Never consume no field. Modules without lambdas add no closure types. Signature-specific environment-first entries and executable allocation/call emission remain pending.
