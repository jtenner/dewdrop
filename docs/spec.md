...
Exact non-generic receiver-free self calls in structural tail position lower to WebAssembly Core 3 `return_call`. Tail positions propagate through block tails, explicit returns, if branches, and match arms; non-tail, generic, receiver, mutual, and callback recursion remain ordinary calls.
