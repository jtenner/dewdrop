...
| D-498 | Implemented provisionally | Expected structural function types may instantiate generic local or imported function references, recording the inferred type arguments with the exact declaration. Program lowering currently executes generic call sites whose body is an exact parameter selector, and transparent generic callback wrappers reduce to the selected callback before that specialization. Arguments retain source evaluation order and non-selected runtime values are dropped. These call-only specializations emit no generic closure or erased adapter; arbitrary generic bodies and escaping generic references still require stable specialization identities and a general callable ABI. |
