...
`dew.std.collections.stack` defines `Stack<t>` as a carrier-preserving standard abstraction over `Array<t>`. `stack_new`, `stack_with_capacity`, `stack_push`, optional `stack_pop`, optional non-mutating `stack_peek`, `stack_length`, `stack_is_empty`, `stack_clear`, and `stack_iter` preserve mutable reference identity. Pop order is LIFO; empty pop and peek return `Option::None`. Stack iteration follows stable insertion order from the bottom element to the current top.
