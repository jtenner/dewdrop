...
| D-563 | Implemented | Fresh struct-variant payload projection follows the same direct and immutable sole-use-local escape proof. A top-level bound pattern field is matched to the constructor field by frozen field identity/name, all constructor fields are appended to a source-ordered selector, and the selected supported carrier is forwarded without variant allocation or pattern dispatch. Nested fields, guards, ambiguous bindings, and non-first-arm proofs remain conservative. |
