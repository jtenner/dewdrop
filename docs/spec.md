...
> **Status:** Static dispatch is implemented. Runtime trait values are implemented through semantic selection and lowering: concrete-to-trait boundaries retain exact coherent evidence, and calls on trait receivers retain exact dynamic requirement slots. WasmGC trait-object construction, dictionaries, adapters, and `call_ref` execution remain pending and are tracked under runtime trait values in `docs/roadmap.md`.
...
| D-539 | Implemented semantically | A bare trait in a value position denotes an erased runtime trait type. Assignability at an expected trait boundary preserves the concrete source type and freezes the exact visible coherent implementation plus recursive prerequisite evidence on the source expression; unresolved source types defer coercion selection until ordinary body constraints settle. Calls on trait-typed receivers freeze a distinct dynamic target containing exact trait and requirement identities, the deterministic source-order receiver-method slot, and instantiated type arguments. Lowering retains coercion independently of the source expression operation and emits `PlannedDynamicTraitCall`; backend dictionary construction and execution remain pending and fail visibly as unsupported. |
