...
| D-536 | Implemented provisionally | Generic `derive(Eq)` synthesizes ordinary conditional Eq and Ne implementations over the applied nominal type. Every generated owner parameter preserves its source bounds and gains one Eq prerequisite unless already present. Field and payload comparisons use symbolic owner-bound evidence inside the generated body; closed concrete uses execute through D-535, nested generic derived values forward prerequisites transitively, and concrete arguments without Eq evidence fail ordinary method selection. |
