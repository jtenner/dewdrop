...
A supported scalar field read directly from fresh struct construction becomes a source-ordered parameter selection over every field initializer, removing the struct allocation and field load without skipping or reordering unselected initializers. Narrow, reference, generic, trait-coerced, variant, let-bound, and escaping aggregates remain unchanged.
...
| D-551 | Implemented provisionally | A supported scalar field projection directly from fresh struct construction is rewritten to `PlannedParameterSelect` over every source-ordered field initializer. Unselected initializers are still evaluated and discarded, the selected initializer becomes the result, and the fresh `struct.new` plus `struct.get` are removed. Narrow storage, reference/generic fields, trait-coerced bases, variants, let-bound aggregates, and escaping values remain unchanged until storage, cast, identity, and escape analysis prove parity. |
