...
The ambient `Iter<t>` protocol has `has_next(self) -> Bool` and state-advancing `next(self) -> t`. Implementations are mutable reference-identity cursors. Calling `next` after exhaustion traps; callers that need a non-trapping boundary must check `has_next` first. Array iteration is stable index order over the live logical prefix. Hash collection iteration order is intentionally unspecified.
