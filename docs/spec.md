...
WasmGC planning emits one closure struct for modules that produce, accept, return, or call first-class function values. Its immutable fields are a nullable abstract function reference and nullable `eqref` environment. Every lambda receives a deterministic environment struct and capture-to-physical-field relocation. Scalar/SIMD captures use compact storage, references use GC-safe `eqref` with exact casts restored at use sites, and Unit/Never consume no field. Modules without first-class function use add no closure types.

Lambda construction allocates the environment and closure, and generated environment-first entry functions receive `(environment, source parameters...)`. Named non-capturing references store their original `ref.func` with a null environment. Invocation evaluates the target once, tests the environment, then uses typed `call_ref` with either the direct signature or environment-first signature. Declarative element segments authorize named and lambda entries. Returned, nested, immediate, passed, module-level, and imported closures execute in Node. Scalar reassignment is not currently a language expression; future mutable capture writes require shared cells rather than copied fields.
...
| D-482 | Implemented semantically | Lambda parameter and result syntax resolves in the enclosing root declaration's generic/type scope. Alias-normalized parameter/results are structurally interned as one canonical function type per lambda. Resolved lambda signature tables are serialized through frozen-interface cache V4; V3 persistent artifacts are rejected and standard-interface filenames use a `v4-` prefix. Isolated lambda-body inference and closure emission remain pending. |
