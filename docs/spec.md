...
> **Status:** Static dispatch and runtime trait values for non-generic object-safe traits over nominal reference receivers are implemented. Concrete-to-trait boundaries retain exact coherent evidence, construct WasmGC trait objects using one shared typed dictionary per implementation, and invoke source-ordered methods with `call_ref`. Imported dictionaries, scalar/SIMD boxes, generic dictionaries, identity/equality, and devirtualization remain tracked in `docs/roadmap.md`.
