...
A call whose target is a module value name reads the frozen source-ordered value binding. One callable declaration selects directly. Function types are structurally resolved, but calls to module lets or local values remain invalid until first-class call-target lowering is implemented. Field and qualified call targets defer to the later implementation/member phase described in Section 3.14.
