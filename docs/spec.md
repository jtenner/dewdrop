...
| D-412 | Superseded by library migration | Text and builder algorithms are ordinary Dew functions. Their dependencies follow normal source calls and imports. Self-host emission has no text-name runtime dispatch or runtime-builder FFI calls. Native erased adapters still use the retired runtime function kind pending its removal. |
