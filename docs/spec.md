...
| D-582 | Implemented | An immutable uncaptured direct local alias may be coalesced into its source local when both locals have identical inferred type, physical shape, and nominal evidence. Alias uses are retargeted, the alias declaration and initializer read are elided, and source initialization stays in place. Scalar, SIMD, and packed source locals may retain other reads because their immutable values cannot change; reference and erased-generic sources remain sole-use until interference planning bounds retention. Mutable locals, captures, conversions, differing nominal evidence, or non-direct aliases remain separate. |
