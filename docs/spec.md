...
`dew.std.show` defines non-ambient `Show` evidence with `append_show(self, builder: StringBuilder) -> Unit`. `show(value)` constructs one builder and returns its finished String; composition helpers append nested shown values, text, or ASCII without intermediate strings. Show is side-effect free and distinct from ambient WASI-backed Debug. Initial stable formats cover Bool, signed/unsigned fixed-width integers, String, and StringView. Postfix `derive(Show)` emits ordinary coherent generic implementations with `Type { field: value }`, `Type::Variant(value)`, and `Type::Variant { field: value }` forms and freezes across imports. Floating-point, Unit, byte, SWAR, SIMD, and explicit recursion-budget formats remain intentionally undefined.
