...
A lambda uses `fn(parameter: Type, ...) -> Result { ... }`. Parameters and result types are initially mandatory, and the body uses the ordinary newline-delimited block grammar. In expression value-seeking mode, `fn` commits directly to lambda parsing; the parser does not rewind or speculate, and the completed block enters ordinary postfix/binary continuation mode. Collection lowers lambda headers immediately and nested blocks into isolated source-ordered HIR fragments with parent/root-body identities. Lambda signature resolution, lexical capture analysis, and executable closure environments remain pending.
