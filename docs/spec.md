...
| D-439 | Implemented | Compiler snapshot diagnostics use explicit `DEW_ERROR` or `DEW_WARNING` framing terminated by `DEW_END`, preserving each possibly multiline deterministic `Debug` representation as one ordered JSON string. The expanded suite contains 158 fixtures: 141 compiled modules and 17 compiler-error modules. |
...
| D-473 | Implemented provisionally | Non-test module-level values receive deterministic mutable Wasm globals. Direct scalar literals use legal constant initializers; other values use dependency-ordered zero-argument initializer functions. Program links order those functions by module dependency SCC and each module's frozen value schedule, then expose one `__dew_init` function that Dew hosts invoke exactly once before `main` or test entry points. Reference globals use nullable `eqref` storage and recover nominal types through frozen casts at use sites. Imported non-callable value access and cross-module eager-value cycle diagnostics remain pending. |
