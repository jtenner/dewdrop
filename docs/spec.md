...
> **Status:** Static dispatch and runtime trait values for object-safe traits over nominal reference, scalar, packed, and SIMD receivers are implemented across linked modules. Concrete-to-trait boundaries retain exact coherent evidence, reconstruct local or imported requirement signatures, construct WasmGC trait objects using one shared typed dictionary per exact evidence tree, and invoke source-ordered methods with `call_ref`. Closed generic implementation dictionaries and transparent-wrapper devirtualization execute; symbolic erasure inside open generic bodies and general escape analysis remain tracked in `docs/roadmap.md`.
...
A non-generic transparent forwarding function whose complete result is one dynamic call on parameter zero may be devirtualized at a direct call site with exact prerequisite-free evidence. The call is rewritten to the concrete implementation method, and the receiver coercion is removed without changing source argument order. General flow-sensitive escape analysis, prerequisite-aware devirtualization, and dead dynamic-wrapper elimination remain optimizations rather than semantic requirements.
