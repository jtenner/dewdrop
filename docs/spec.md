...
| D-579 | Implemented | A nonconstant guard on a known matching constructor may be eliminated for tuple or struct variants when it is a bounded pure Boolean tree of local/capture reads, constants, logical negation, conjunction, and disjunction, and every possible fallthrough arm for that constructor returns the identical direct payload binding. Struct fields are compared through frozen field-name/layout identity. Since either guard outcome selects the same source-ordered constructor component, guard evaluation and match dispatch are observationally redundant. Effectful guards, differing payloads, catchalls, unsupported operators, or missing fallback remain conservative. |
