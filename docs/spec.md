...
Opaque nominal brands and private foreign-library namespaces use:
...
`pub foreign type` exports only the nominal Dew brand. A foreign library
namespace and its members cannot be `pub`. Members are monomorphic static
functions: they require explicit parameter and result types, have no receiver
or body, and bind one exact guest export string. Calls use the namespace alias,
for example `Starshine.module_new()`.
...
| D-615 | Implemented | `dew.std.wasm.wasi` is the complete low-level WASI Preview 1 package: all 46 functions use exact `wasi_snapshot_preview1` imports and explicit U32/U64/I64 carrier aliases, while callers own linear-memory records and buffers. The generic builtin-import classifier recognizes the full WASI prefix instead of two named functions. Any reachable raw WASI import exports memory 0. `dew.std.wasi` preserves the existing bounded GC-Bytes read/write adapter and retains only its two private imports, so programs using only that adapter do not retain the complete raw signature surface. One Node/Wago fixture invokes every function; both hosts validate all parameter/result counts and complete call coverage, and an architecture case requires 46 exact WASI imports with no accidental `dew`-namespace prefix. |
