...
| D-503 | Implemented provisionally | A demanded scalar specialization of a public root generic whose generic occurrences are direct parameters/results receives one deterministic `<source>$dew$<carrier,...>` export adapter. Five lazily emitted immutable WasmGC boxes carry `i32`, `i64`, `f32`, `f64`, and `v128`; adapters box scalar arguments before invoking the all-`eqref` fallback and cast/unbox scalar results afterward. Ordinary Dew calls and function references retain allocation-free concrete specializations. Nested generic occurrences inside aggregates or structural function signatures are intentionally excluded until recursive representation adapters are defined. |
