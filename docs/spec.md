...
| D-439 | Implemented | Compiler snapshot diagnostics use explicit `DEW_ERROR` or `DEW_WARNING` framing terminated by `DEW_END`, preserving each possibly multiline deterministic `Debug` representation as one ordered JSON string. The expanded suite contains 165 fixtures: 146 compiled modules and 19 compiler-error modules. |
