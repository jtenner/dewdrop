...
| D-600 | Implemented | `dew.std.wasm.intrinsics` is generated from the complete backend inline-builtin name set and the exact existing Dew carrier signatures that select those names. The generator emits 1,312 unambiguous public aliases covering 829 backend names, preserves the original `i64_trunc_i32` spelling, embeds identical bootstrap bytes, rejects duplicate aliases, and fails if any backend inline name lacks a declaration. Direct Node/Wago execution and a host-independent WAT budget cover scalar arithmetic/comparison, truncation, float unary math, linear memory, and SIMD. |
