...
| D-439 | Implemented | Compiler snapshot diagnostics use explicit `DEW_ERROR` or `DEW_WARNING` framing terminated by `DEW_END`, preserving each possibly multiline deterministic `Debug` representation as one ordered JSON string. The expanded suite contains 166 fixtures: 146 compiled modules and 20 compiler-error modules. |
...
| D-482 | Implemented semantically | Lambda parameter and result syntax resolves in the enclosing root declaration's generic/type scope. Alias-normalized parameter/results are structurally interned as one canonical function type per lambda. Resolved lambda signature tables are serialized through frozen-interface cache V5; V4 and older persistent artifacts are rejected and standard-interface filenames use a `v5-` prefix. Isolated lambda-body inference and closure emission remain pending. |
...
| D-497 | Implemented provisionally | Program lowering recognizes a conservative transparent callback summary: a callable whose entire result is `callback(parameter_1, ...)`, where callback is parameter zero and every remaining parameter is forwarded once in source order. A direct call passing a known lambda or named function is rewritten to the callback target, eliminating wrapper dispatch and closure construction. Any extra work, reordering, unused argument, control flow, local storage, return/storage escape, or unknown callback value disables this rewrite. |
...
| D-506 | Implemented provisionally | Aggregate boundary conversion descends through acyclic nested generic struct fields. Each nested object is evaluated once into a deterministic `eqref` scratch local, recursively reconstructed from converted leaves, and inserted into its parent clone. Nominal cycles are detected during link planning and remain excluded rather than receiving an incorrect finite clone; enum payload recursion and structural-function wrapper closures remain pending. |
| D-507 | Implemented provisionally | Every exported frozen-interface callable carries a 256-bit lowercase hexadecimal ABI fingerprint. The V1 preimage domain-separates callable, erased-box, and flattened-closure schema versions and encodes module path, source export name, generic arity, normalized parameter/result type trees, and physical value shapes. Generic parameters use declaration-relative ordinals; exact nominal/trait/Self identities remain encoded by current `DeclId` pending dependency-interface fingerprint substitution. Frozen interfaces and standard-interface cache envelopes advance to V5, and V4 filenames/artifacts are never decoded as V5. |
