...
| D-482 | Implemented and cache-refined | Lambda signatures resolve in the enclosing generic/type scope and persist through the current V12 frozen-interface/cache format. Isolated body inference and executable closure emission are implemented by later decisions. |
...
| D-507 | Superseded by D-524, D-525, and the V12 interface/cache decisions | Exported callables now use V4 fingerprints, public nominals and module content use content-sensitive V2 fingerprints, transitive interfaces ignore assigned semantic IDs, and `dew.abi` performs compatibility negotiation. |
...
| D-526 | Superseded in package syntax by D-527 | Versioned external packages, integrity/interface validation, deterministic dependency ordering, and V12 dependency-interface cache behavior remain implemented. Resolution records moved out of the package definition into `dew.lock`. |
