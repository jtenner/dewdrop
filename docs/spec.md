...
A supported scalar field read directly from fresh struct construction becomes a source-ordered parameter selection over every field initializer, removing the struct allocation and field load without skipping or reordering unselected initializers. The same applies to an immutable uncaptured local whose sole use is an immediately following direct field read; its declaration and local read are elided. Narrow, reference, generic, trait-coerced, variant, mutable/captured, multiple-use, non-adjacent, and escaping aggregates remain unchanged.
...
| D-551 | Implemented provisionally | A supported scalar field projection from fresh struct construction is rewritten to `PlannedParameterSelect` over every source-ordered field initializer. The same rewrite applies to an immutable uncaptured local when its sole use is the immediately following direct field read; the declaration and local read are elided from copied lowering arenas. Unselected initializers are still evaluated and discarded, while `struct.new` and `struct.get` disappear. Narrow storage, reference/generic fields, trait-coerced bases, variants, mutable/captured locals, multiple/non-adjacent uses, and escaping values remain unchanged. |
