...
> **Status:** Static dispatch and runtime trait values for non-generic object-safe traits over nominal reference, scalar, packed, and SIMD receivers are implemented across linked modules. Concrete-to-trait boundaries retain exact coherent evidence, reconstruct local or imported requirement signatures, construct WasmGC trait objects using one shared typed dictionary per implementation, and invoke source-ordered methods with `call_ref`. Generic prerequisite dictionaries and devirtualization remain tracked in `docs/roadmap.md`.
