...
An array literal is `[` followed by zero or more comma-delimited expressions and `]`; newlines and one trailing comma are accepted. Elements evaluate from left to right and unify with one element type. `[]` requires an expected `Array<t>` type. Collection lowers the literal to a block with an ordinary `array_with_capacity` call, ordered `push` calls, and the resulting array local. A private exact import binds the constructor independently of local values or types named `Array`; no open import is required. The ordinary library constructor allocates exact capacity once, and its declared fields and methods control storage. Later calls can also supply an empty literal's element type. Product elements are boxed only because one Array slot requires one carrier; indexed reads unbox them to their logical multivalue shape. Implementation notes are in `docs/research/product-types-and-array-literals-2026-08-18.md` and `docs/research/array-literal-calls-2026-09-14.md`.
...
| D-412 | Superseded by library migration | Text and builder algorithms are ordinary Dew functions. Their dependencies follow normal source calls and imports. Both compilers and the provider have no runtime-builder protocol or runtime function kind. Version-1 caches reject the retired function-kind tag 2. Native erased adapters have their own function kind and explicit, unique-target checks. |
