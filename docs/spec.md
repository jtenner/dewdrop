...
| D-582 | Implemented | An immutable uncaptured direct local alias may be coalesced into its source local when the alias initializer is the source's only read and both locals have identical inferred type, physical shape, and nominal evidence. Alias uses are retargeted, the alias declaration and initializer read are elided, and source initialization stays in place. Shared source reads, mutable locals, captures, conversions, differing nominal evidence, or non-direct aliases remain separate. |
