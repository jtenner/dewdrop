...
`dew.std.queue` defines mutable FIFO `Queue<t>` with construction, exact reservation, enqueue, optional dequeue, optional non-mutating front peek, length, emptiness, clear, and explicit iterator functions. Empty dequeue and peek return `Option::None`; aliases observe mutation; iteration is stable front-to-back. Its current contiguous implementation remains a measured O(n) baseline pending the next bounded representation-only rebase onto the circular-buffer runtime.
