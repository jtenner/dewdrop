...
Return types are required, including explicit `-> Unit`. One trailing comma is accepted in type-parameter and value-parameter lists. Generic parameter bounds are not accepted initially, but this decision may be reopened if a required builtin cannot be expressed safely without them. Signature types may be named or recursively applied generic types such as `Result<List<I32>>`.
...
A call whose target is a module value name reads the frozen source-ordered value binding. One callable declaration selects directly. Calls to module lets or local values are currently invalid because first-class function types are not yet implemented. Field and qualified call targets defer to the later implementation/member phase described in Section 3.14.
...
`container[key]` selects the ambient `Index<key, value>` trait and produces `value`. `container[key] = value` selects `IndexSet<key, value>` but is a block item rather than an expression: it cannot be nested, returned, passed as an argument, or used as a tail value. The parser first completes the left expression, observes `=` with its sole token lookahead, requires the outer node to be an index, and then parses the value without adding `=` to the expression precedence table. Receiver, key, and value execute once in that order.
...
`IndexSet` is distinct from a reference-returning `IndexMut` because Dew has no mutable-reference or borrow abstraction. Until associated types exist, the compiler must treat `(Self, key) -> value` as a functional dependency; complete cross-module enforcement remains pending. Implementation and validation details are recorded in `docs/research/fixed-array-indexing.md`.
...
The initial deterministic WasmGC representation uses sixteen buckets with separate collision chains. Map entries store their U64 hash plus the canonical six erased carrier slots for both key and value; Set reuses the same private bucket/entry machinery while leaving the value carriers unused. Hash executes once per operation; `hash_eq` executes only after stored-hash equality. Selection freezes both exact implementation method identities before lowering, linking marks them reachable, and emission performs no trait lookup. Traversal is iterative, including Set head/middle/tail unlinking. Iteration order is unspecified and no iterator API is exposed yet. Geometric bucket growth, Map removal, and compact specialized entry layouts remain future work. Full details are recorded in `docs/research/hash-map.md`.
...
| D-306 | Implemented | Callable function types follow physical GC types in stable callable order without signature deduplication. |
...
| D-337 | Implemented | Frozen interfaces include public aggregate fields and variants, while translated imported implementation evidence remains separate from the local coherence/dispatch index until cross-module impl merging is specified. |
...
| D-346 | Implemented | Program physical types are rebased by dependency-SCC/module order while local recursive groups retain `TypeIdx::rec`; cross-module forward type references are diagnosed until global physical SCC merging exists. |
...
| D-439 | Implemented | Compiler snapshot diagnostics use explicit `DEW_ERROR` or `DEW_WARNING` framing terminated by `DEW_END`, preserving each possibly multiline deterministic `Debug` representation as one ordered JSON string. The expanded suite contains 135 fixtures: 123 compiled modules and 12 compiler-error modules. |
...
| D-447 | Implemented provisionally | `tools/dew check` and `tools/dew build -o OUTPUT` accept explicitly ordered files grouped by repeated `--module`, select a static link root with `--root`, execute the immutable program pipeline, preserve framed deterministic diagnostics, return nonzero status on failure, and emit Starshine-validated deterministic WasmGC. `tools/dew test` supports explicitly ordered files and strict manifests, self-describing V3 metadata, multi-module static linking, stable filters, expected traps, and listing. All three commands resolve import-selected on-disk `dew.std` sources through ordered package roots. `dew run`, installed binaries, and file-aware diagnostic rendering remain pending. |
| D-448 | Implemented provisionally | The initial strict UTF-8 `dew.json` manifest contains exactly `root` plus ordered `{ name, files }` module records. Check/build/test reject duplicate keys/modules/files, unknown or missing fields, undeclared roots, empty arrays, absolute/escaping/out-of-workspace/non-`.dew`/missing sources, and never enumerate the filesystem. External dependency declarations, package versions, and user-package discovery remain pending. |
...
| D-463 | Implemented | Indexed reads select ambient `Index<key, value>` evidence. Indexed setting uses `IndexSet<key, value>` and is a block item rather than an assignment expression; receiver, key, and value evaluate once in that order. |
| D-464 | Implemented provisionally | `dew.std.fixed_array` uses one nominal wrapper over carrier-specialized mutable WasmGC arrays. `get` returns `Option<t>`; index get/set, `set`, `get_unchecked`, and `set_unchecked` trap out of bounds. Complete functional-dependency coherence and growable Array remain pending. |
...
| D-467 | Implemented provisionally | `dew.std.map` is a mutable deterministic WasmGC separate-chaining map with sixteen buckets, erased unboxed key/value carrier slots, safe optional lookup, trapping index lookup, insertion/replacement, membership, length, and indexed setting. Growth, Map removal, and iteration remain pending. |
| D-468 | Implemented provisionally | `dew.std.set` reuses the collision-safe Hash table representation and exact frozen Hash evidence. It provides empty/singleton construction, length, emptiness, membership, Boolean insert/remove, and O(1) logical clear; insertion is idempotent and removal unlinks collision-chain heads, middles, and tails deterministically. Growth and iteration remain pending. |
