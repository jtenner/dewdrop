...
> Living roadmap as of September 14, 2026. This file records current product direction. Completed work is summarized compactly; detailed implementation history and measurements live in [`docs/spec.md`](spec.md), [`docs/architecture.md`](architecture.md), and [`docs/research/`](research/). [`agent-todo.md`](../agent-todo.md) is the execution-only backlog and records active work and its prerequisites.
...
- fixed-width scalar, packed-lane, SIMD, strict UTF-8 text, Bytes, strict bounded JSON, WASI Preview 1, capability-based Facet 0.1 host adapters, test metadata, and deterministic Starshine validation/encoding;
...
2. [x] Consume the pinned Starshine submodule's generated raw WasmGC FFI through a generated compiler subset (currently 473 selected exports of 4,214 available), typed container/result bridges, and a BLAKE3 compiler fingerprint over the pinned revision, interface digests, selected signatures, and exact provider bytes; build a linked source-Bytes smoke compiler that emits byte-identical validated modules.
3. [ ] Port the parser, semantic phases, optimizer, backend, cache codecs, and compiler driver to Dew in dependency order. The tokenizer, full parser, packed semantic IDs, declaration collection, flat type/body HIR, namespace construction, duplicate diagnostics, multi-file collection, canonical type interning, local and imported signature resolution, alias SCC diagnostics, local and imported generic alias substitution, transparent normalization, public signature interfaces, exact/open/wildcard type, trait, and value imports, two-stage cyclic interface freezing, lexical body names, pattern bindings, loop controls, lambda captures, deterministic Eq/Ne/Debug/Hash/Show derive expansion, frozen callable/aggregate/implementation recipes, consumer-local imported semantic recipes, rollback-capable local type unification, the basic per-body inference core, iterative generic signature instantiation, local direct callable/overload selection, local fields, struct construction, inherent methods, all local enum construction forms, local unit/tuple/struct variant patterns, trait-backed operators, index selection, imported generic calls, imported generic fields, imported constructors and variants, imported unit/tuple/struct patterns, imported generic inherent methods, imported free-function overload sets, implementation-head indexing, orphan checks, generic overlap checks, duplicate inherent-method checks, trait method signature conformance, generic specialization ranking, coherent local method dispatch, imported implementation evidence merging, and concrete trait-evidence selection, recursive implementation prerequisite checks, and root body evidence publication are ported. Declared body-bound evidence nodes, full prerequisite expression-root evidence attachment, WasmGC fragment planning, physical link planning, the canonical physical body-carrier plan, and linked Starshine module emission are ported into the source-Bytes smoke compiler. Compiler A builds, validates, executes, and produces compiler B; B produces byte-identical compiler C. The latest bootstrap also passes with ordinary Array literal calls and Starshine `983dfb180` plus generated-export fix `de52f1d14`; see [`research/array-literal-calls-2026-09-14.md`](research/array-literal-calls-2026-09-14.md). Runtime failures are being converted into compile-time gates, and the first formal proof foundations for arena spans, physical carriers, and dense type remaps are in `src/compiler_proofs` (see [`research/self-host-formal-proof-foundation-2026-08-31.md`](research/self-host-formal-proof-foundation-2026-08-31.md)). Semantic lowering is complete for all current HIR forms, including local/imported builtin identities, nominal identities, body/lambda locals and captures, control targets, selections, shapes, flow, patterns, calls, assignments, evidence roots, and initialization/function/layout inputs. Program lowering now aggregates modules in dependency-first SCC order. Backend-neutral specialization planning now closes local/imported generic call demands transitively and deduplicates canonical physical shape vectors. WasmGC fragment planning publishes nominal physical type families, field storage, callable ABI value types, elided generic bases, and local/imported specialization clones, and the linked Starshine emission port emits closures, dictionaries, recursive physical SCCs, runtime dependencies, globals, instructions, and fresh final indexes. Concrete local/imported prerequisite evidence trees with deterministic children, shape arguments, depth bounds, and rollback are ported. Deterministic lambda/body inference job merging, type-ID rebasing, and stack-safe merged body value-shape planning are ported. Local callable ABI planning and nominal type layout planning are ported. Alternative-pattern flow coverage is complete. Invalid patterns now suppress secondary flow coverage diagnostics. Dependency-first module initialization planning, resolved backend value-shape classification, and local nominal type layouts are ported. Module-value dependencies, dependency-first SCC scheduling, shared-SCC inference with imported selections, and frozen graph import into independent function solvers are ported. Successful basic body inference arenas are compacted; diagnostic-bearing bodies retain their original IDs conservatively. The persistent parse-event artifact format remains with the later cache-codec port.
4. [x] Require the fixed-point bootstrap: the MoonBit compiler builds compiler A, compiler A builds compiler B, compiler B builds compiler C, and B/C are byte-identical.
