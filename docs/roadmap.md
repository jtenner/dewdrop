...
- [x] Implement the ordered set wrapper and range API over the shared red-black tree core.
...
- [x] Reuse repeated scalar field extraction from the same immutable direct local when an earlier lexical `let` already stores the value; elide duplicate uncaptured aliases without adding scratch locals. Also reuse exact nominal-reference field aliases as bases for nested scalar extraction while excluding mutable, generic, nominal-unknown, and effect-uncertain cases.
- [x] Coalesce immutable uncaptured direct local aliases into their exact source local when type, carrier, and nominal evidence match, preserving initialization and mutable snapshot semantics. Scalar, SIMD, and packed carriers may share other immutable source reads; reference and generic carriers remain sole-use until interference planning bounds retention.
- [ ] Extend fresh tuple/struct-variant payload elimination beyond nested aggregate bindings, constructor-resolved alternatives, exact constant guards, stable scalar/composite/payload-dependent nonconstant guard equivalence for tuple and struct variants, false catchall interception proofs, scalar-constant catchall selection, exact binding-forwarding, nested binding projection, direct and let-bound exact tuple/struct primitive scalar transforms, and constructor-disjoint preceding arms to broader binding-derived scalar bodies and nonconstant guard planning; nonescaping singleton boxes and exact-flow trait objects already eliminate wrappers and dispatch artifacts.
...
- [x] Extend scalar replacement beyond fresh `if`/match joins and complete/reordered/one-missing distinct-field aliases to direct multi-missing and mixed alias/direct aggregates with synthesized scalar, fresh/local/module exact nominal-reference, and direct erased-generic component locals; repeated reads reuse the aggregate local, while coerced generic/reference planning remains conservative.
