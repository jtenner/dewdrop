...
- [ ] Complete postfix `derive(...)` for easy system traits. Structs/enums support deterministic `derive(Eq)` and `derive(Debug)` expansion through ordinary coherent implementations; non-generic Debug recursively streams source-ordered fields/payloads and Bool/fixed-width integers through ambient `debug(value)`, while generic value evidence, complete text/bytes escaping, lane formats, `Show`, `Hash`, visibility, and cross-module ABI behavior remain.
