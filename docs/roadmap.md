...
- [ ] Extend fresh tuple/struct-variant payload elimination beyond nested aggregate bindings, constructor-resolved alternatives, exact constant guards, stable scalar/composite/payload-dependent nonconstant guard equivalence for tuple and struct variants, false catchall interception proofs, scalar-constant catchall selection, exact binding-forwarding, nested binding projection, exact primitive scalar transforms, and constructor-disjoint preceding arms to broader binding-derived scalar bodies and nonconstant guard planning; nonescaping singleton boxes and exact-flow trait objects already eliminate wrappers and dispatch artifacts.
