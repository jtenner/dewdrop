...
- [x] Extend scalar replacement beyond fresh `if`/match joins and complete/reordered/one-missing distinct-field aliases to direct multi-missing and mixed alias/direct aggregates with synthesized scalar and exact nominal-reference component locals; repeated reads reuse the aggregate local, while erased-generic planning remains conservative.
