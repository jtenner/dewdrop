...
- [x] Extend scalar replacement beyond fresh `if`/match joins and complete/reordered/one-missing distinct-field aliases to direct multi-missing and mixed alias/direct aggregates with synthesized scalar, exact nominal-reference, and direct erased-generic component locals; repeated reads reuse the aggregate local, while fresh/coerced generic planning remains conservative.
