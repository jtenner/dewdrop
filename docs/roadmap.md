...
- [ ] Define ABI/interface fingerprints after specialization and erasure rules stabilize. V2 SHA-256 callable fingerprints now persist through frozen-interface/cache V6 and use stable dependency module path, declaration name, and generic arity rather than external `DeclId`; content-sensitive nominal fingerprints and compatibility negotiation remain. A Node host consumer lists deterministic adapter exports and invokes scalar carrier boundaries. Focused in-Wasm consumers now exercise `i32` callback-bearing struct and enum adapters, a standalone exact-nominal `eqref` fallback, and the JavaScript-inaccessible `v128` identity adapter; imported-package consumers remain.
