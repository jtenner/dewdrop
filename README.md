...
- Carrier-specialized `FixedArray`, growable `Array`, bounded/growable circular buffers, comparator-driven `BinaryHeap`, `PriorityQueue`, red-black trees, ordered maps, ordered sets, and builder-backed Unit/primitive and derived `Show` formatting, and explicit `Disposable` cleanup evidence, and deterministic LIFO `defer`, scoped `using`, and fixed-point primitive constant folding and immutable-summary scalar inlining, Core 3 direct tail recursion, and immutable scalar-field common-subexpression elimination and fresh, pure-gap sole-use, repeated-same-field, ordered component-local, fresh-if-join, and nonescaping reference/generic box and fresh tuple-variant payload scalar replacement, double-ended `Deque`, LIFO `Stack`, FIFO `Queue`, and mutable `Map` and `Set` collections with explicit iterators.
