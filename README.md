...
- `FixedArray`, `Map`, and `Set` standard-library collections.
...
The returned lambda **captures** `current`. Because `current` is mutable, the compiler stores it in one shared runtime cell so later calls see the updated value.
