...
- Carrier-specialized `FixedArray`, growable `Array`, bounded/growable circular buffers, comparator-driven `BinaryHeap`, `PriorityQueue`, red-black trees, ordered maps, ordered sets, and builder-backed primitive and derived `Show` formatting, and explicit `Disposable` cleanup evidence, and deterministic LIFO `defer` and scoped `using`, double-ended `Deque`, LIFO `Stack`, FIFO `Queue`, and mutable `Map` and `Set` collections with explicit iterators.
