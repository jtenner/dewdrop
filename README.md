...
Multi-module programs use repeated `--module NAME` arguments or strict `dew.json` manifests. Manifests may declare a package `{ name, version }` and versioned dependencies with exact relative manifest paths, `sha256-<hex>` source integrity, and expected 64-hex transitive interface fingerprints; `tools/dew package-integrity PATH/TO/dew.json` computes the canonical package digest. Dependency manifests resolve transitively before compilation, reject identity/version/integrity conflicts and cycles, and participate in persistent V7 dependency-interface caching. Compiler-owned standard sources are loaded from deterministic registered paths; `--bootstrap-std` selects generated fallback sources.
