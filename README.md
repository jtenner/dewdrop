...
- Versioned `dew.abi` interface-compatibility metadata plus `dew.metrics` static-cost metadata and deterministic specialization, adapter, allocation-site, dispatch, cast, and output-size budgets.
...
Packages use a convention-first `dew.json` containing only `@scope/name`, exact package version, and dependency requests. Exact versions, Git sources, materialized paths, SHA-256 source integrity, and expected interface fingerprints live separately in `dew.lock`. A package derives one Dew module from its package name and collects sorted `src/**/*.dew`, or immediate sibling `*.dew` when `src/` is absent; `_test.dew` is selected only by `dew test`. Running `tools/dew` from the package directory discovers `dew.json` automatically. `tools/dew package-integrity PATH` computes canonical package integrity. Bootstrap compiler fixtures may still use explicit `dew.modules.json` root/module/file graphs or repeated `--module NAME` arguments. Compiler-owned standard sources are loaded from deterministic registered paths; `--bootstrap-std` selects generated fallback sources.

Programs with non-constant module-level values export `__dew_init`. Dew-owned hosts call it exactly once after instantiation and before `main` or test entry points, allowing initialization to use host imports without relying on a WebAssembly start section. Public module-level `let` values are available through `open` and `@alias.value`; their inferred scalar, nominal, and applied generic types are frozen into module interfaces. Public non-foreign implementation evidence is likewise frozen with exact method identities, while test-only, private-head, and `foreign impl` evidence remains module-local. Consumer coherence rejects overlapping local/imported or independently imported trait implementations before dispatch. Reachable nominal dependencies close across modules, and mutually recursive physical WasmGC types are emitted as deterministic shared recursive groups. First-class functions use structural `fn(T, ...) -> R` types and a WasmGC closure ABI. Explicit typed lambdas may capture lexical values, nest, escape through returns, pass as parameters, initialize module values, and cross module boundaries. Named non-generic functions and lambdas share one program-wide linked closure base while calls retain typed direct or environment-first `call_ref` signatures. Uncaptured `let mut` bindings remain unboxed Wasm locals; captured mutable bindings use one carrier-specialized shared WasmGC cell visible to the declaring body and all direct, nested, sibling, returned, or directized closures. Non-generic overload sets are selected as values from exact structural expected types across local and imported candidates. Generic function references infer deterministic physical-carrier specializations from exact structural expected types.
...
Dew is under active development. The compiler and standard library are usable for the supported subset. Generic functions execute through deterministic program-wide physical-carrier specialization, including structured bodies, nested generic calls, multiple type parameters, imported definitions, and escaping function references. Public root-module generics also expose one nullable-`eqref` fallback. Exact nominal function references receive deterministic static-to-erased adapters only when their callable signatures differ, while demanded scalar specializations export deterministic `$dew$<carrier>` adapters that box arguments into immutable WasmGC carrier objects and unbox results. Generic structs and enums use deterministic erased carrier fields and execute without scalar boxes during ordinary specialized execution. Boundary adapters recursively reconstruct nested and cyclic nominal values and use flattened closure subtypes to bridge structural callback parameters, results, nested callbacks, and function-valued struct or enum payload fields. Focused in-Wasm consumers now construct callback-bearing generic struct and enum values, invoke their `$dew$i32` adapters, call the reconstructed wrappers, and round-trip an exact nominal value through a public `eqref` fallback without exposing GC references to JavaScript. An in-Wasm vector consumer also invokes `identity$dew$v128` and extracts its returned lane without asking JavaScript to represent a SIMD argument or result. The same structurally equivalent consumer also links against a multi-module provider whose root adapter exposes an imported generic callback aggregate; the host validates the provider's schema versions, module path, and transitive interface fingerprint before instantiation. Content-sensitive nominal ABI fingerprints, compatibility negotiation, package TOML/lockfiles, growable arrays, queues, iterators, and several tooling surfaces remain planned.
