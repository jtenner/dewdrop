...
- Carrier-specialized `FixedArray`, growable `Array`, bounded/growable circular buffers, comparator-driven `BinaryHeap` and `PriorityQueue`, double-ended `Deque`, LIFO `Stack`, FIFO `Queue`, and mutable `Map` and `Set` collections with explicit iterators.
