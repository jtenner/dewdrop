...
This separates compilation from execution. It is useful when testing a different runtime or embedding the module in another host program. Successful Wasm, HIR, and lowering builds are stored as verified content-addressed entries under `.dew/cache/builds/`; an exact repeated request can restore the output without invoking the compiler. Compilation also stores checksummed per-file parser-event artifacts under `.dew/cache/parse-events/`, reused by import scanning and semantic collection, non-root workspace frozen interfaces under `.dew/cache/workspace-interfaces/`, and complete inferred module-body artifacts under `.dew/cache/body-inference/`. Workspace interface keys include exact manifest source identity and direct public dependency fingerprints; body keys add exact module source plus the transitive frozen interface fingerprint. Private dependency changes therefore retain downstream interface and body reuse when the public interface is unchanged, while public interface or implementation-evidence changes invalidate dependents. Experimental `--body-family-cache` mode additionally stores atomic declaration-family bundles under `.dew/cache/body-inference-families/`, allowing an edited root body to miss while unchanged root bodies and their nested lambdas are rebased and reused. This layer is opt-in because current compactness measurements do not justify default JSON decoding overhead. Use `--no-build-cache` to force compilation, `--no-parse-event-cache` to force parsing, `--no-interface-cache` to disable frozen-interface reuse, `--no-body-cache` to disable both body-inference layers, and `--cache-report` to report each cache layer.
