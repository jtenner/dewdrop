...
push_value :: ∀ a. RPN a -> a -> RPN a
push_value (RPN stack ops) v = spy "pushing value" RPN (v : stack) ops

infixl 4 push_value_maybe as ++?
push_value_maybe :: ∀ a. Maybe (RPN a) -> a -> Maybe (RPN a)
...
pop_binary (x : y : xs) f = Just (f x y : xs)
...
finalize (RPN (x : Nil) Nil) = Just x

-- unary operators can be evaluated with the top item on the stack
finalize (RPN (v : stack) (LeftUnary f : ops)) = finalize (RPN (f v : stack) ops)
finalize (RPN (v : stack) (RightUnary f : ops)) = finalize (RPN (f v : stack) ops)

-- binary operatores require two operands
finalize (RPN (v1 : v2 : stack) (Binary _ _ g : ops)) = finalize (RPN (g v1 v2 : stack) ops)

-- pass groups through
finalize (RPN stack (Group : ops)) = finalize (RPN stack ops)
