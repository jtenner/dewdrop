...
import Data.Bits (BitReader, BitsMut, read_string, read_u8, read_utf8_char, write_8, write_char, write_string)
...
import Util (to_signed)

type EncoderFn a = BitsMut -> a -> BitsMut
type DecoderFn a = BitReader -> Maybe (Tuple a BitReader)

encode_vec :: ∀ (@u :: Type). EncoderFn u -> FingerTree u -> BitsMut -> BitsMut
...
encode_maybe :: ∀ (@u :: Type). EncoderFn u -> Maybe u -> BitsMut -> BitsMut
...
decode_sleb128 s = go 0 0 (read_u8 s)
  where
  go :: Int -> Int -> Maybe { head :: Int, tail :: BitReader } -> Maybe (Tuple Int BitReader)
  go _ _ Nothing = Nothing
  go acc count (Just { head: byte, tail: s' })
    | count >= 10 = Nothing
...
encode_uleb128 :: Int -> BitsMut -> BitsMut
encode_uleb128 v s = go 0 v s
  where
  go :: Int -> Int -> BitsMut -> BitsMut
  go count v' s'
    | count >= 10 = s'
    | v' <= 0x7F = write_8 v' s'
    | otherwise = do
...
encode_sleb128 :: Int -> BitsMut -> BitsMut
...
encode_char :: Char -> BitsMut -> BitsMut
encode_char = write_char

decode_char :: BitReader -> Maybe (Tuple Char BitReader)
decode_char reader = do
  { head, tail } <- read_utf8_char reader
  pure $ Tuple head tail

encode_string :: String -> BitsMut -> BitsMut
encode_string v s =
  let
...
decode_string s = do
  ...
  { head: str, tail: s'' } <- read_string byte_length s'
  pure $ Tuple str s''


encode_u8 :: Int -> BitsMut -> BitsMut
encode_u8 = write_8

decode_u8 :: BitReader -> Maybe (Tuple Int BitReader)
decode_u8 bits = do
  { head, tail } <- read_u8 bits
  pure $ Tuple head tail

encode_lanes :: Lanes -> BitsMut -> BitsMut
...
encode_locals :: Locals -> BitsMut -> Maybe BitsMut
...
encode_heap_type :: HeapType -> BitsMut -> Maybe BitsMut
...
decode_heap_type s = case read_u8 s of
  Just { head: 0x73, tail: s' } -> Just $ Tuple HeapTypeNoFunc s'
  Just { head: 0x72, tail: s' } -> Just $ Tuple HeapTypeNoExtern s'
  Just { head: 0x71, tail: s' } -> Just $ Tuple HeapTypeNone s'
  Just { head: 0x70, tail: s' } -> Just $ Tuple HeapTypeFunc s'
  Just { head: 0x6F, tail: s' } -> Just $ Tuple HeapTypeExtern s'
  Just { head: 0x6E, tail: s' } -> Just $ Tuple HeapTypeAny s'
  Just { head: 0x6D, tail: s' } -> Just $ Tuple HeapTypeEq s'
  Just { head: 0x6C, tail: s' } -> Just $ Tuple HeapTypeI31 s'
  Just { head: 0x6B, tail: s' } -> Just $ Tuple HeapTypeStruct s'
  Just { head: 0x6A, tail: s' } -> Just $ Tuple HeapTypeArray s'
  _ -> case decode_sleb128 s of
    Just (Tuple index s') | index >= 0 -> Just $ Tuple (HeapTypeIndex (TypeIndex index)) s'
    _ -> Nothing

encode_ref_type :: RefType -> BitsMut -> Maybe BitsMut
...
decode_ref_type s = case read_u8 s of
  Just { head: 0x73, tail: s' } -> Just $ Tuple RefTypeNoFunc s'
  Just { head: 0x72, tail: s' } -> Just $ Tuple RefTypeNoExtern s'
  Just { head: 0x71, tail: s' } -> Just $ Tuple RefTypeNone s'
  Just { head: 0x70, tail: s' } -> Just $ Tuple RefTypeFunc s'
  Just { head: 0x6F, tail: s' } -> Just $ Tuple RefTypeExtern s'
  Just { head: 0x6E, tail: s' } -> Just $ Tuple RefTypeAny s'
  Just { head: 0x6D, tail: s' } -> Just $ Tuple RefTypeEq s'
  Just { head: 0x6C, tail: s' } -> Just $ Tuple RefTypeI31 s'
  Just { head: 0x6B, tail: s' } -> Just $ Tuple RefTypeStruct s'
  Just { head: 0x6A, tail: s' } -> Just $ Tuple RefTypeArray s'
  Just { head: 0x63, tail: s' } -> case read_u8 s' of
    Just { head: 0x73, tail: s'' } -> Just $ Tuple RefTypeNoFunc s''
    Just { head: 0x72, tail: s'' } -> Just $ Tuple RefTypeNoExtern s''
    Just { head: 0x71, tail: s'' } -> Just $ Tuple RefTypeNone s''
    Just { head: 0x70, tail: s'' } -> Just $ Tuple RefTypeFunc s''
...
decode_val_type s = case read_u8 s of
  Just { head: 0x7F, tail: s' } -> Just $ Tuple ValTypeI32 s'
  Just { head: 0x7E, tail: s' } -> Just $ Tuple ValTypeI64 s'
  Just { head: 0x7D, tail: s' } -> Just $ Tuple ValTypeF32 s'
  Just { head: 0x7C, tail: s' } -> Just $ Tuple ValTypeF64 s'
  Just { head: 0x7B, tail: s' } -> Just $ Tuple ValTypeV128 s'
  _ -> do
    Tuple rt s' <- decode_ref_type s
    Just $ Tuple (ValTypeRefType rt) s'

encode_val_type :: ValType -> BitsMut -> Maybe BitsMut
